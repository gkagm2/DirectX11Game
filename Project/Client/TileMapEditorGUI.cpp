    ...
    if (ImGui::Begin(STR_GUI_TileMapEditor, &m_bGUIOpen))
    {
		...
		if (ImGui::Button("Create")) {
			if (0 != m_arrFaceTileCnt[0] &&
				...
				m_pTileMap->CreateTile(m_arrFaceTileCnt[0], m_arrFaceTileCnt[1]);
			}
		}
		...
		ImGui::DragFloat("Size", &fTileBtnSize, 0.2f, 2.0f, 100.0f, "%.0f");

		int iTexCol = m_pTileMap->GetAtlasTileCol();
		int iTexRow = m_pTileMap->GetAtlasTileRow();

		vector<TTileInfo>& vecTilesInfo = m_pTileMap->GetTilesInfo();

		/*for (int i = 0; i < iTexRow; ++i) {
			for (int j = 0; j < iTexCol; ++j) {
				_RenderTileMap("TileMap&&Texture", m_pTileMap->GetAtlasTexture().Get(), this, (GUI_CALLBACK)&TileMapEditorGUI::_SelectTileMap, vecTilesInfo[i * iTexCol + j]);
			}
			ImGui::Spacing();
		}*/

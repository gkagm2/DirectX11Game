...
void CFBXLoader::LoadMaterial(FbxSurfaceMaterial* _pMtrlSur)
{
	...
	// Diff
	tMtrlInfo.tMtrl.vDiffuse = GetMtrlData(_pMtrlSur
		, FbxSurfaceMaterial::sDiffuse
		, FbxSurfaceMaterial::sDiffuseFactor);

	// Amb
	tMtrlInfo.tMtrl.vAmbiant = GetMtrlData(_pMtrlSur
		, FbxSurfaceMaterial::sAmbient
		, FbxSurfaceMaterial::sAmbientFactor);

	// Spec
	tMtrlInfo.tMtrl.vSpecular = GetMtrlData(_pMtrlSur
		, FbxSurfaceMaterial::sSpecular
		, FbxSurfaceMaterial::sSpecularFactor);

	// Emisv
	tMtrlInfo.tMtrl.vEmission = GetMtrlData(_pMtrlSur
...
{
	path path_content = CPathManager::GetInstance()->GetContentPath();
	...
	for (UINT i = 0; i < m_vecContainer.size(); ++i)
	{
		for (UINT j = 0; j < m_vecContainer[i].vecMtrl.size(); ++j)
		{
			...
			for (size_t k = 0; k < vecPath.size(); ++k)
			{
				...
				path_dest = CPathManager::GetInstance()->GetRelativePath(path_dest.c_str());
				CResourceManager::GetInstance()->LoadRes<CTexture>(path_dest, path_dest);
...
void CFBXLoader::CreateMaterial()
{
	...
	for (UINT i = 0; i < m_vecContainer.size(); ++i)
	{
		for (UINT j = 0; j < m_vecContainer[i].vecMtrl.size(); ++j)
		{
			...
			strPath = STR_DIR_PATH_Material;
			strPath += strKey;

			// 상대경로가 곧 이름(확장자명은 제외)
			m_vecContainer[i].vecMtrl[j].strMtrlName = strKey;
			pMaterial->SetKey(strKey);
			pMaterial->SetRelativePath(strPath + STR_EXTENSION_Mtrl);
			

			pMaterial->SetShader(CResourceManager::GetInstance()->FindRes<CGraphicsShader>(STR_KEY_Std3DDeferredShader));

			wstring strTexKey = m_vecContainer[i].vecMtrl[j].strDiff;
			SharedPtr<CTexture> pTex = CResourceManager::GetInstance()->FindRes<CTexture>(strTexKey);
			if (NULL != pTex)
				pMaterial->SetData(E_ShaderParam::Texture_0, pTex.Get());

			strTexKey = m_vecContainer[i].vecMtrl[j].strNormal;
			pTex = CResourceManager::GetInstance()->FindRes<CTexture>(strTexKey);
			if (NULL != pTex)
				pMaterial->SetData(E_ShaderParam::Texture_1, pTex.Get());

			strTexKey = m_vecContainer[i].vecMtrl[j].strSpec;
			pTex = CResourceManager::GetInstance()->FindRes<CTexture>(strTexKey);
			if (NULL != pTex)
				pMaterial->SetData(E_ShaderParam::Texture_2, pTex.Get());

			pMaterial->SetMaterialCoefficient(
				m_vecContainer[i].vecMtrl[j].tMtrl.vDiffuse
				, m_vecContainer[i].vecMtrl[j].tMtrl.vSpecular
				, m_vecContainer[i].vecMtrl[j].tMtrl.vAmbiant
				, m_vecContainer[i].vecMtrl[j].tMtrl.vEmission);

			CResourceManager::GetInstance()->AddRes<CMaterial>(pMaterial->GetKey(), pMaterial);
		}
	}
}

...
void CS_WeightMap(int3 _iThreadID : SV_DispatchThreadID)
{
    ...
    // 증가 량
    float fAdd = g_fDeltaTime * vBrushColor.a;
...
#ifndef _WEIGHTMAP_FX
#define _WEIGHTMAP_FX
...
void CS_WeightMap(int3 _iThreadID : SV_DispatchThreadID)
{
    ...
    if (_iThreadID.x < vCenterPos.x - (vScale.x * 0.5f) || _iThreadID.x > vCenterPos.x + (vScale.x * 0.5f) || 
        _iThreadID.y < vCenterPos.y - (vScale.y * 0.5f) || _iThreadID.y > vCenterPos.y + (vScale.y * 0.5f))
        return;
    
    // brush texture로 높이 설정
    int2 vLTPos = vCenterPos - (vScale * 0.5f);
    float2 vUV = float2(_iThreadID.xy - vLTPos) / float2(vScale);
        
    //float4 vBrushColor = BRUSH_TEX.SampleLevel(Sample_Anisotropic, float3(vUV, BRUSH_IDX), 0);
    ...
    // 가중치 맵 수정
    //WEIGHT_MAP[iIdx].arrWeight[1] += g_fDeltaTime * 0.5f * vBrushColor.a;
    
    // 현재 가중치 
    float arrWeight[4] = WEIGHT_MAP[iIdx].arrWeight;
    
    // 증가량
    float fAddMnt = g_fDeltaTime * vBrushColor.a;
    
    // 가중치를 올리고, 나머지 부위를 비율에 맞추어 내린다.
    arrWeight[WEIGHT_IDX] = saturate(arrWeight[WEIGHT_IDX] + fAddMnt);
    ...
    if (1.f < fTotal)
    {
        ...
        for (int i = 0; i < 4; ++i)
        {
            if (i == WEIGHT_IDX)
                continue;
            // 초과량을 비율에 맞추어 나눠서, 다른 가중치 쪽에서 뺀다.
            arrWeight[i] = saturate(arrWeight[i] - fOver * (arrWeight[i] / fTotal));
        }
    }
